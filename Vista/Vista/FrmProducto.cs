...
namespace Vista
{
    public partial class FrmProducto : MetroFramework.Forms.MetroForm
    {
        ...
        public void establecerValores(int pId, string pName, int sId, int cId,
            double uPrice, int uStock, int reorderLevel, bool discont)
        {
            proId = pId;
            reoLevel = reorderLevel;
            if (pId != 0)
            {
            ...
            else
            {
                if (proId == 0)
                {
                    prdt = new Product(0, txtNombre.Text, int.Parse(cmbCompania.SelectedValue.ToString()), cmbCategoria.SelectedText,
                            int.Parse(cmbCategoria.SelectedValue.ToString()), cmbCategoria.SelectedText, Convert.ToDouble(txtPrecio.Text), int.Parse(txtStock.Text), reoLevel, cbxDescontinuado.Checked);
